...
public class Main {
	...
	void menu() {
		System.out.println("SunibGedung\r\n"
				+ "====================\r\n"
				+ "1. Insert New Menu\r\n"
				+ "2. View Menu\r\n"
				+ "3. Update Menu\r\n"
	...
	void insertDatatoDatabase(Vector<Menu> menus) {
			...
			if(rs.next()) {
				...
				for (int i = 0; i < menus.size(); i++) {
					...
					case "Appetizer":
						...
						Connect.getInstance().insertData(menus.get(i).getID(),menus.get(i).getName(), menus.get(i).getPrice(), menus.get(i).getQuantity(), category, temp1.getType());
						break;
					case "MainCourse":
						MainCourse temp2 = (MainCourse)menus.get(i);
						Connect.getInstance().insertData(menus.get(i).getID(),menus.get(i).getName(), menus.get(i).getPrice(), menus.get(i).getQuantity(), category, temp2.getType());
						break;
					case "Dessert":
						Dessert temp3 = (Dessert)menus.get(i);
						Connect.getInstance().insertData(menus.get(i).getID(),menus.get(i).getName(), menus.get(i).getPrice(), menus.get(i).getQuantity(), category, temp3.getTopping());
						break;
					case "Drink":
						Drink temp4 = (Drink)menus.get(i);
						Connect.getInstance().insertData(menus.get(i).getID(),menus.get(i).getName(), menus.get(i).getPrice(), menus.get(i).getQuantity(), category, temp4.getSize());
						break;
					}
				}
	...
	public void insertMenu(Vector<Menu> menus) {
		...
		GetFactoryMenu factory = new GetFactoryMenu();
		Menu p =factory.orderMenu(cat, id, name, pri, quan, type); 
		menus.add(p);
		Connect.getInstance().insertData(id, name, pri, quan, cat, type);
	...
	void updateMenu(Vector<Menu> menus) {
		...
		if(category.equals("Appetizer")) {
			...
			String query = "UPDATE menus SET Name = "+name+",Price = "+pri+",Quantity = "+quan+",Category = "+category+",Description = "+type+" WHERE ID = "+ID;
		...
		if(category.equals("Dessert") ) {
			...
			menus.setElementAt(temp, idx);
			String query = "UPDATE menus SET Name = "+name+",Price = "+pri+",Quantity = "+quan+",Category = "+category+",Description = "+topping+" WHERE ID = "+ID;
			Connect.getInstance().execUpdate(query);
		}
		...
		if(category.equals("Drink") ) {
			...
			temp.setSize(size);
			menus.setElementAt(temp,idx);
			String query = "UPDATE menus SET Name = '"+name+"',Price = "+pri+",Quantity = "+quan+",Category = '"+category+"',Description = '"+size+"' WHERE ID = '"+ID+"'";
			Connect.getInstance().execUpdate(query);
		}
	...
	public Main() throws IOException {
		Vector<Menu> menus = new Menu(){}.data();
		Vector<Order> orders = new Vector<>();
		int pil=-1;
//		insertDummyData(menus);
		insertDatatoDatabase(menus);
		invoice=menus.size();
		count=invoice+1;
